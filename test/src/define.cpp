...
string strtohex(string value)
{
	...
	{
		sprintf(szBuf, "%02X", (unsigned char) value[i]);
		result += szBuf;
	}
	return result;
}
...
string getchksum(string data)
{
	...
	{
		value = value + hextoint(strtohex(data.substr(i, 1)));
	}
	value = value % 65535;
	value = ~value;
	value = value + 1;
	return inttohex(value, 8).substr(4, 4);
}
...
string get_lenchecksum(string data)
{
	...
	for(unsigned int i=0;i<data.length();i++){
		value += (unsigned char)strtol(data.substr(i, 1).c_str(), 0, 16);
	}
	value = ~(value % 16)+1;
	char buf[2];
	sprintf(buf, "%02X", value);
	return string(buf+1)+data;
}

void fileCopy(string filePath)
{
	ofstream out;
	out.open("test.dav", ios::out|ios::binary);
